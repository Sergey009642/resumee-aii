...
import styles from "./ResumeTemplate3.module.scss";
...
const ResumeTemplate3 = React.forwardRef<HTMLDivElement, { resumeData: ResumeData; photo?: File | null; allowEditing?: boolean; contentSpace?: number }>(
  ({ resumeData, photo }, ref) => {
    ...
    return (
      <div ref={ref} className={styles.gridLayout}>
        <div className={styles.gridHeader}>
          <div className={styles.headerPhotoWrap}>
            {photoUrl ? (
              <img src={photoUrl} alt="Resume photo" className={styles.photo} />
            ) : (
              <div className={styles.photo} style={{ background: '#e0e7ef', color: '#64748b', fontSize: 18 }}>Photo</div>
            )}
          </div>
